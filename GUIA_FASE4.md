...
```
Cliente → :8080 (Gateway) → lb://ms-gestion-instructor (:8081)
                          → lb://ms-gestion-alumno     (:8082)
                          → lb://ms-gestion-taller      (:8083)
```
...
- `File > New > Module from Existing Sources...` y selecciona
  `ms-admin-api-gateway/pom.xml`, **o** click derecho sobre ese `pom.xml` → *Add as Maven Project*.
- Espera a que descargue dependencias.

## 2) Reinicia el Config Server
Cambió `ms-admin-api-gateway-dev.yml` (ahora trae rutas + CORS). Como está en el
classpath del Config Server, **reinicia el Config Server** para que lo recargue.
Verifica: `http://localhost:8888/ms-admin-api-gateway/dev` → debe mostrar las `routes`.

## 3) Orden de arranque
1. ms-admin-config-server (8888)
2. ms-admin-registry-server (8761)
3. ms-gestion-instructor / alumno / taller (8081/8082/8083)
4. **ms-admin-api-gateway (8080)**  ← arráncalo al final
...
```
Debe listar las 3 rutas (`ms-gestion-instructor`, `ms-gestion-alumno`, `ms-gestion-taller`).
